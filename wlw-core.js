...
void (async function () {
    ...
    try {
        ...
        for (const entry of fetched) {
            ...
            state.roles[entry.id] = entry.role;
        }
    ...
    async function fetchCastInFrame(id) {
        ...
        while (Date.now() - started < 3000) {
            try {
                return { data: readCastData(castDoc), rank: readCastRank(castDoc), role: readCastRole(castDoc) };
            } catch (error) {
                lastError = error;
    ...
    async function collectOtherCasts(roster, currentId) {
        ...
        for (let i = 0; i < roster.ids.length; i++) {
            ...
            try {
                ...
                fetched.push({ id, data: result.data, rank: result.rank, role: result.role });
            } catch (error) {
                throw new Error(
                    "一括取得を中止しました。\n" +
